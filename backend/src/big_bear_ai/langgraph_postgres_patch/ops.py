...
from big_bear_ai.langgraph_postgres_patch.checkpoint import Checkpointer
from big_bear_ai.langgraph_postgres_patch.database import connect, pool_stats as database_pool_stats
from big_bear_ai.langgraph_postgres_patch.redis import (
    CHANNEL_RUN_CONTROL,
    CHANNEL_RUN_STREAM,
    LIST_RUN_QUEUE,
    LOCK_RUN_STATS,
...
from big_bear_ai.langgraph_postgres_patch.retry import RETRIABLE_EXCEPTIONS, RetryableException

logger = structlog.stdlib.get_logger(__name__)
BG_JOB_INTERVAL = float(os.getenv("BG_JOB_INTERVAL", "1"))
...
def _snapshot_defaults() -> dict[str, tuple[Any, ...]]:
...
class TTLSweepResult(NamedTuple):
    expired: int
    deleted: int
# noqa  MC80OmFIVnBZMlhsdktEbHY1ZnBtNFE2YnpKeGFBPT06N2Y1MWIzOTk=
...
class Assistants(Authenticated):
    resource = "assistants"
# type: ignore  MS80OmFIVnBZMlhsdktEbHY1ZnBtNFE2YnpKeGFBPT06N2Y1MWIzOTk=
    ...
    async def search(
        ...
        *,
        ...
        if name:
            query += " AND LOWER(name) LIKE %(name)s"
...
# noqa  Mi80OmFIVnBZMlhsdktEbHY1ZnBtNFE2YnpKeGFBPT06N2Y1MWIzOTk=
    ...
    @staticmethod
    async def delete(
        conn: AsyncConnection[DictRow] | None,
        assistant_id: UUID,
        ctx: Auth.types.BaseAuthContext | None = None,
        *,
        delete_threads: bool = False,
    ) -> AsyncIterator[UUID]:
        """Delete an assistant and optionally its assistant-owned threads."""
        async with AsyncExitStack() as stack:
            ...
            authorized = await conn.execute(
                f"SELECT assistant_id FROM assistant WHERE assistant_id = %(assistant_id)s{filter_clause}",
                params,
                binary=True,
            )
            if await authorized.fetchone() is None:
                raise HTTPException(
                    status_code=404,
            ...
            if delete_threads:
                thread_rows = await conn.execute(
                    "SELECT thread_id FROM thread WHERE metadata ->> 'assistant_id' = %(assistant_id)s",
                    {"assistant_id": str(assistant_id)},
                    binary=True,
                )
                for row in await thread_rows.fetchall():
                    async for _ in await Threads.delete(conn, row["thread_id"], ctx=ctx):
                        pass
            ...
            cur = await conn.execute(
                "DELETE FROM assistant WHERE assistant_id = %(assistant_id)s RETURNING assistant_id",
                {"assistant_id": assistant_id},
                binary=True,
            )
            rows = await cur.fetchall()

            async def deleted() -> AsyncIterator[UUID]:
                for row in rows:
                    yield row["assistant_id"]
...
# pylint: disable  My80OmFIVnBZMlhsdktEbHY1ZnBtNFE2YnpKeGFBPT06N2Y1MWIzOTk=
...
class Threads(Authenticated):
        ...
        *,
        ...
        async def _row_iter():
            ...
            for row in rows:
                ...
                if extract:
                    result["extracted"] = {
                        alias: extract_path_value(result, path)
    ...
    async def _get(
        ...
        *,
        ...
        filters = {**(read_filters or {}), **(filters or {})}
        filter_clause, filter_params = _build_filter_query(
            filters=filters, table_alias="thread"
        )
        selected_columns = (
            ", ".join(f"thread.{path}" for path in read_mask_paths)
        ...
        ttl_join = ""
        if include_ttl:
            ttl_join = " LEFT JOIN thread_ttl ON thread_ttl.thread_id = thread.thread_id"
            ttl_column = """,
                jsonb_strip_nulls(jsonb_build_object(
                    'strategy', thread_ttl.strategy,
                    'ttl_minutes', thread_ttl.ttl_minutes,
                    'expires_at', thread_ttl.expires_at
                )) AS ttl"""
        cur = await conn.execute(
            f"SELECT {selected_columns}{ttl_column} FROM thread{ttl_join} "
            f"WHERE thread.thread_id = %(thread_id)s{filter_clause}",
            {"thread_id": thread_id, **filter_params},
            binary=True,
        )
        return (row async for row in cur)
    ...
    async def get(
        ...
        return await Threads._get(
            conn,
            thread_id,
            ctx=ctx,
            include_ttl=include_ttl,
            read_mask_paths=read_mask_paths,
        )
    ...
    async def patch(
        ...
        *,
        ...
        returning_columns = (
            ", ".join(read_mask_paths) if read_mask_paths is not None else "*"
        )
        cur = await conn.execute(
            "\n".join(
                (
    ...
    async def set_status(
        ...
        await Threads._set_status(conn, thread_id, checkpoint, exception)
    ...
    async def prune(
        ...
        if strategy != "delete":
            raise HTTPException(
                status_code=422,
                detail="keep_latest strategy is not supported by the PostgreSQL runtime",
            )

        normalized_ids = [UUID(str(thread_id)) for thread_id in thread_ids]
        filters = await Threads.handle_event(ctx, "delete", {"thread_ids": normalized_ids})
        filter_clause, filter_params = _build_filter_query(filters=filters, table_alias="thread")
        async with connect() as conn:
            selected = await conn.execute(
                f"SELECT thread_id FROM thread WHERE thread_id = ANY(%(thread_ids)s){filter_clause}",
                {"thread_ids": normalized_ids, **filter_params},
                binary=True,
            )
            allowed_ids = [row["thread_id"] for row in await selected.fetchall()]
            if filters and len(allowed_ids) != len(normalized_ids):
                raise HTTPException(
                    status_code=404,
                    detail="At least one thread not found or not authorized",
                )
            return await Threads._delete_bulk(conn, allowed_ids, batch_size=batch_size)
        ...
        async def bulk(
            ...
            async with conn.pipeline():
                thread, run_graph_id = await asyncio.gather(
                    Threads._get(conn, thread_id, ctx=ctx, filters=filters),
                    conn.execute(
                        "select kwargs -> 'config' -> 'configurable' ->> 'graph_id' as graph_id from run where thread_id = %(thread_id)s limit 1",
                        {"thread_id": config["configurable"]["thread_id"]},
                    ),
...
class Runs(Authenticated):
    ...
    async def next(wait: bool, limit: int = 1) -> AsyncIterator[tuple[Run, int]]:
        ...
        for idx, run in enumerate(runs):
            ...
            run["kwargs"].setdefault("checkpoint_during", True)
            run["kwargs"].setdefault("durability", "async")
    ...
    async def enter(
        ...
        async with (
            await get_pubsub(
                channels=[CHANNEL_RUN_CONTROL.format(thread_id, run_id)],
            ) as pubsub,
    ...
    @staticmethod
    async def pool_stats() -> dict[str, dict[str, int]]:
        return cast(dict[str, dict[str, int]], database_pool_stats(None, None, format="json"))
    ...
    async def put(
        ...
        *,
        ...
        if langsmith_session_name is not None:
            kwargs["langsmith_session_name"] = langsmith_session_name
        kwargs.setdefault("config", {})
        kwargs.setdefault("checkpoint_during", True)
...
updated_thread AS (
    ...
    async def cancel(
        ...
        *,
        ...
        where_parts = []
        if assistant_id is not None:
            where_parts.append("run.assistant_id = %(assistant_id)s")
            where_parts.append("run.status IN ('pending', 'running')")
        elif status_list:
            placeholders = ", ".join(f"'{value}'" for value in status_list)
    ...
    class Stream(Authenticated):
        ...
        async def subscribe(
            ...
            pubsub = await get_pubsub(
                ...
                patterns=[CHANNEL_RUN_STREAM.format(thread_id, run_id) + ":*"],
            )
            return pubsub
            ...
            *,
            ...
            try:
                ...
                if last_event_id is not None:
                    ...
                    for stream_id, fields_list in events:
                        event = fields_list[3]
                        message = fields_list[5]
                        event_name = event.decode()
                        if highest_stream_id is None or _compare_stream_ids(
                            stream_id, highest_stream_id
                        ) > 0:
                            highest_stream_id = stream_id
                        if (
                            not stream_mode
                            ...
                            or (
                                {"messages", "messages-tuple"}.intersection(stream_mode)
...
{authorized_thread_cte}inserted_cron AS (
    INSERT INTO cron (
        ...
        schedule, payload, next_run_date, metadata, on_run_completed, enabled, timezone
    )
    SELECT
...
LIMIT 1
        ...
        params = {
            ...
            "next_run_date": next_cron_date(schedule, datetime.now(UTC), timezone=timezone),
            "metadata": Jsonb(metadata),
    ...
    async def get(
        ...
        cron_id = UUID(str(cron_id))
        filters = await Crons.handle_event(ctx, "read", Auth.types.CronsRead(cron_id=cron_id))
        filter_clause, filter_params = _build_filter_query(filters=filters, table_alias="cron")
        cur = await conn.execute(
            f"SELECT * FROM cron WHERE cron_id = %(cron_id)s{filter_clause}",
            {"cron_id": cron_id, **filter_params},
            binary=True,
        )
        rows = await cur.fetchall()

        async def results() -> AsyncIterator[Cron]:
            for row in rows:
                yield {**row, "payload": json_loads(row["payload"])}
    ...
    async def update(
        ...
        *,
        ...
        if not any(value is not None for value in (schedule, end_time, enabled, on_run_completed, payload, metadata, timezone)):
        ...
        stored_payload = json_loads(row["payload"])
        next_payload = {**stored_payload, **(payload or {})}
        next_payload["assistant_id"] = stored_payload["assistant_id"]
        configurable = next_payload.setdefault("config", {}).setdefault("configurable", {})
        configurable["cron_id"] = str(cron_id)
        next_schedule = schedule if schedule is not None else row["schedule"]
        next_timezone = timezone if timezone is not None else row["timezone"]

        set_fields = ["updated_at = NOW()", "payload = %(payload)s"]
        params: dict[str, Any] = {"cron_id": cron_id, "payload": Jsonb(next_payload), **filter_params}
        for field, value in (("schedule", schedule), ("end_time", end_time), ("enabled", enabled), ("on_run_completed", on_run_completed), ("timezone", timezone)):
            if value is not None:
                set_fields.append(f"{field} = %({field})s")
                params[field] = value
        if metadata is not None:
            set_fields.append("metadata = cron.metadata || %(metadata)s")
            params["metadata"] = Jsonb(metadata)
        if schedule is not None or timezone is not None:
            set_fields.append("next_run_date = %(next_run_date)s")
            params["next_run_date"] = next_cron_date(next_schedule, datetime.now(UTC), timezone=next_timezone)

        cur = await conn.execute(
            f"UPDATE cron SET {', '.join(set_fields)} WHERE cron_id = %(cron_id)s{filter_clause} RETURNING *",
            params,
            binary=True,
        )
        rows = await cur.fetchall()
        ...
        async def results() -> AsyncIterator[Cron]:
            for updated in rows:
                yield {**updated, "payload": json_loads(updated["payload"])}
    ...
    async def search(
        ...
        *,
        ...
        if enabled is not None:
            filter_enabled = " AND cron.enabled = %(enabled)s"
            filter_params["enabled"] = enabled
        else:
            filter_enabled = ""
        if metadata:
            filter_metadata = " AND cron.metadata @> %(metadata)s"
            filter_params["metadata"] = Jsonb(metadata)
        else:
            filter_metadata = ""
        query = f"""
